...
type CitationStyle = 'Harvard' | 'Vancouver' | 'None' | 'APA' | 'Chicago' | 'AMA' | 'AP' | 'Canadian';
	...
	async onload() {
		...
		this.addCommand({
			...
			editorCallback: async (editor: Editor, view: MarkdownView) => {
				...
				new CitationStyleModal(this.app, async (newStyle) => {
					try {
...
class CitationStyleModal extends Modal {
	...
	constructor(app: App, onSubmit: (newStyle: CitationStyle) => void) {
		super(app);
		this.onSubmit = onSubmit;
	}
	...
	onOpen() {
		...
		new Setting(contentEl)
			...
			.addDropdown(async (dropdown: DropdownComponent) => {
				const citationStyles: CitationStyle[] = ['Harvard', 'Vancouver', 'APA', 'Chicago', 'AMA', 'AP', 'Canadian', 'None'];
				citationStyles.forEach(style => {
					dropdown.addOption(style, style);
				});
				dropdown.setValue('Harvard'); // Default value
